...
public class DepartmentFrame extends JFrame {
    ...
    private void loadDepartments() {
        try {
            ...
            for (Department d : list) {
                model.addRow(new Object[]{d.getDepartmentId(), d.getName(), d.getCode()});
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Failed to load departments:\n" + ex.getMessage(),
    ...
    private void openAddDialog() {
        AddDepartmentDialog dlg = new AddDepartmentDialog(this);
        dlg.setVisible(true);
        loadDepartments(); // refresh after dialog closes
    }
    ...
    private void deleteSelectedDepartment() {
        ...
        String deptName = String.valueOf(model.getValueAt(row, 1));

        try {
            // ✅ Block delete before attempting SQL delete
            if (departmentDAO.hasEmployees(deptId)) {
                JOptionPane.showMessageDialog(this,
                        "Cannot delete department \"" + deptName + "\".\nEmployees are currently assigned to it.",
                        "Delete Blocked", JOptionPane.WARNING_MESSAGE);
                return;
            }

            if (departmentDAO.hasHistory(deptId)) {
                JOptionPane.showMessageDialog(this,
                        "Cannot delete department \"" + deptName + "\".\nIt is referenced in department history.",
                        "Delete Blocked", JOptionPane.WARNING_MESSAGE);
                return;
            }
